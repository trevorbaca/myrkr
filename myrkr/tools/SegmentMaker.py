...
class SegmentMaker(makertools.SegmentMaker):
    ...
    def _attach_first_segment_default_instruments(self, score_package):
        if not self._is_first_segment():
            return
    ...
    def _interpret_music_handler(self, music_handler):
        if isinstance(music_handler.scope, baca.tools.SimpleScope):
            compound_scope = baca.tools.CompoundScope(music_handler.scope)
    ...
    def copy_music_maker(self, _context_name, _stage, **kwargs):
        r'''Copies music-maker.
        
        Gets music-maker with `_context_name` defined for `_stage`. Then makes
        new music-maker from this with optional `kwargs`.

        Short-cut for get-then-new.

        Uses private positional argument names `_context_name` and `_stage` to
        avoid aliasing public keyword argument names `context_name` and
        `stage`.
